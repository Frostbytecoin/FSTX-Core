...
#if defined(NDEBUG)
#error "Frostbyte cannot be compiled without assertions."
#endif
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zFSTX spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zFSTX spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zFSTX spend with serial %s from tx %s is not in valid range\n",
    ...
    int64_t nSubsidy = 0;
      ...
      if (nHeight == 0) {
             nSubsidy = 1000000 * COIN;
                ...
                nSubsidy = 1.5 * COIN;
      } else if (nHeight <= 69001 && nHeight > 1000) {
                nSubsidy = 12 * COIN;
      } else if (nHeight <= 86401 && nHeight > 69001) {
                nSubsidy = 20 * COIN;
      } else if (nHeight <= 95401 && nHeight > 86401) {
                nSubsidy = 40 * COIN;
      } else if (nHeight <= 107401 && nHeight > 95401) {
                nSubsidy = 30 * COIN;
      } else if (nHeight <= 127201 && nHeight > 107401) {
                nSubsidy = 20 * COIN;
      } else if (nHeight <= 172801 && nHeight > 127201) {
                nSubsidy = 10 * COIN;
      } else if (nHeight <= 202801 && nHeight > 172801) {
                nSubsidy = 45 * COIN;
      } else if (nHeight <= 209801 && nHeight > 202801) {
                nSubsidy = 60 * COIN;
      } else if (nHeight <= 216801 && nHeight > 209801) {
                nSubsidy = 55 * COIN;
      } else if (nHeight <= 276801 && nHeight > 216801) {
                nSubsidy = 35 * COIN;
      } else if (nHeight <= 446801 && nHeight > 276801) {
                nSubsidy = 15 * COIN;
      } else {
		nSubsidy = 5 * COIN;
      }
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Frostbyte
...
{
    RenameThread("frostbyte-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZFROSTBYTEMinted()
{
...
void RecalculateZFROSTBYTESpent()
{
    CBlockIndex* pindex = chainActive[Params().Zerocoin_AccumulatorStartHeight()];
    while (true) {
        if (pindex->nHeight % 1000 == 0)
        ...
        //Remove spends from zFSTX supply
...
bool RecalculateFROSTBYTESupply(int nHeightStart)
{
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zFSTX with serial %s is already in the block %d\n",
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zFSTXSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert FROSTBYTE to zFSTX
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zFSTX spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zFSTX serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zFSTX mints and %d zFSTX spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Frostbyte: We use certain sporks during IBD, so check to see if they are
