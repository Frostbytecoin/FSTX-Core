...
// Copyright (c) 2017 The Frostbyte developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#if defined(HAVE_CONFIG_H)
#include "config/frostbyte-config.h"
#endif
...
void PrepareShutdown()
{
    ...
    RenameThread("frostbyte-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "frostbyte.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "frostbyted.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the FROSTBYTE and zFSTX money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 22211, 5520));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in FROSTBYTE/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in FROSTBYTE/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, frostbyte, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in FROSTBYTE/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Frostbyte specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:22211"));
    ...
    strUsage += HelpMessageOpt("-backupzfrostbyte=<n>", strprintf(_("Enable automatic wallet backups triggered after each zFSTX minting (0-1, default: %u)"), 1));

//    strUsage += "  -anonymizefrostbyteamount=<n>     " + strprintf(_("Keep N FROSTBYTE anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i The Frostbyte Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    RenameThread("frostbyte-loadblk");
...
/** Sanity checks
 *  Ensure that Frostbyte is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Frostbyte Core is shutting down."));
...
#endif
    // Make sure only a single Frostbyte process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Frostbyte Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Frostbyte version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                // Frostbyte: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() >= Params().Zerocoin_AccumulatorStartHeight()) {
                        RecalculateZFROSTBYTEMinted();
                        RecalculateZFROSTBYTESpent();
                    }
                    RecalculateFROSTBYTESupply(1);
                }
                ...
                // Frostbyte: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Frostbyte Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart Frostbyte Core to complete") << "\n";
        ...
        bool fEnableZFrostbyteBackups = GetBoolArg("-backupzfrostbyte", true);
        pwalletMain->setZFrostbyteAutoBackups(fEnableZFrostbyteBackups);
    }  // (!fDisableWallet)
#else  // ENABLE_WALLET
...
//    }
//
//    nAnonymizeFrostbyteAmount = GetArg("-anonymizefrostbyteamount", 0);
//    if (nAnonymizeFrostbyteAmount > 999999) nAnonymizeFrostbyteAmount = 999999;
//    if (nAnonymizeFrostbyteAmount < 2) nAnonymizeFrostbyteAmount = 2;
    ...
    LogPrintf("Anonymize Frostbyte Amount %d\n", nAnonymizeFrostbyteAmount);
    ...
    /* Denominations
       ...
       For example:
       1FROSTBYTE+1000 == (.1FROSTBYTE+100)*10
       10FROSTBYTE+10000 == (1FROSTBYTE+1000)*10
    */
