...
#include "ui_zfrostbytecontroldialog.h"
...
std::list<std::string> ZFrostbyteControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZFrostbyteControlDialog::listMints;

ZFrostbyteControlDialog::ZFrostbyteControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZFrostbyteControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZFrostbyteControlDialog::~ZFrostbyteControlDialog()
{
    delete ui;
}

void ZFrostbyteControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZFrostbyteControlDialog::updateList()
{
...
void ZFrostbyteControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZFrostbyteControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZFrostbyte_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZFrostbyteControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZFrostbyteControlDialog::GetSelectedMints()
{
...
void ZFrostbyteControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
