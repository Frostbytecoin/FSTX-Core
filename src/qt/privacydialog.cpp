...
#include "zfrostbytecontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zFSTX ought to be enough for anybody." - Bill Gates, 2017
    ui->zFSTXpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzFSTXSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzFSTX->setEnabled(false);
        ui->pushButtonMintzFSTX->setToolTip(tr("zFSTX is currently disabled due to maintenance."));

        ui->pushButtonSpendzFSTX->setEnabled(false);
        ui->pushButtonSpendzFSTX->setToolTip(tr("zFSTX is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zFSTXpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzFSTX_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zFSTX is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zFSTX...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zFSTX in ") +
...
void PrivacyDialog::on_pushButtonSpendzFSTX_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zFSTX is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zFSTX
        sendzFSTX();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zFSTX
    sendzFSTX();
}

void PrivacyDialog::on_pushButtonZFrostbyteControl_clicked()
{
    ZFrostbyteControlDialog* zFSTXControl = new ZFrostbyteControlDialog(this);
    zFSTXControl->setModel(walletModel);
    zFSTXControl->exec();
}

void PrivacyDialog::setZFrostbyteControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzFSTXSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzFSTX()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Frostbyte Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zFSTXpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zFSTX is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " FROSTBYTE </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zFSTX</b>";
    ...
    // use mints from zFSTX selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZFrostbyteControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZFrostbyteControlDialog::GetSelectedMints();
    }
    ...
    // Clear zfrostbyte selector in case it was used
    ZFrostbyteControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zFSTX Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Frostbyte, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zFSTX Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zFSTX </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zFSTX "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zFSTX "));
    ui->labelzFSTXAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzFSTXSyncStatus->setVisible(fShow);
}
